...
"""Fused swap-AB NVFP4-weight/NVFP4-activation fc1+fc2 kernel for SM120."""
...
# =============================================================================
# Sm120SwapABSwigluNvfp4Fc12Kernel
# =============================================================================


class Sm120SwapABSwigluNvfp4Fc12Kernel:
    """Fused swap-AB NVFP4 fc1+fc2 SwiGLU grouped GEMM for MoE on SM120.
    ...
    def __init__(
        ...
        sf_vec_size: int = NVFP4_BLOCK,
        scenario: Literal["2Dx3D"] = "2Dx3D",
        *,
        weight_dtype: Type[cutlass.Numeric] = cutlass.Float4E2M1FN,
        activation_dtype: Type[cutlass.Numeric] = cutlass.Float4E2M1FN,
        ...
        if sf_vec_size != NVFP4_BLOCK:
            raise NotImplementedError(
                f"SM120 NVFP4 requires sf_vec_size={NVFP4_BLOCK}; "
                f"got {sf_vec_size}."
            )
        if weight_dtype is not cutlass.Float4E2M1FN:
            raise NotImplementedError(
                "SM120 NVFP4 requires Float4E2M1FN weights; "
                f"got {weight_dtype}."
            )
        if activation_dtype is not cutlass.Float4E2M1FN:
            raise NotImplementedError(
                "SM120 NVFP4 requires Float4E2M1FN activations; "
    ...
    def _validate_mma_tiler_and_cluster_shape(self) -> None:
        """Validate user-provided geometry for the SM120 NVFP4 swap-AB path."""
        m, n, k = self.mma_tiler_mnk
        cm, cn = self.cluster_shape_mn

        if m != 64:
            raise ValueError(f"SM120 NVFP4 swap-AB starts with mma_tiler M=64; got {m}.")

        if n not in (16, 32, 64, 128):
            raise ValueError(
                "SM120 NVFP4 swap-AB supports N in (16,32,64,128); "
                f"got {n}."
            )

        if k % 64 != 0:
            raise ValueError(
                f"SM120 NVFP4 K ({k}) must be a multiple of the m16n8k64 K atom."
            )
    ...
    def _make_sm120_operand_smem_layout(
        ...
        *,
        ...
        fields such as ``a_major_mode``. SM120 NVFP4 warp MMA does not expose
    ...
    def _setup_attributes(self) -> None:
        ...
        if self.sf_dtype is not cutlass.Float8E4M3FN:
            raise TypeError(
                f"NVFP4 scale dtype must be Float8E4M3FN, got {self.sf_dtype}."
            )
        if self.fc1_output_dtype is not cutlass.Float4E2M1FN:
            raise TypeError(
                "NVFP4 FC1 output dtype must be Float4E2M1FN, got "
                f"{self.fc1_output_dtype}."
            )
        ...
        if self.enable_token_comm and getattr(self, "world_size", 1) == 1:
            ...
            # NVFP4 K1 remains at one CTA/SM with either two or three AB stages.
    ...
    def _compute_stages(
        ...
        # TMA/ldmatrix data layouts and also miss NVFP4 scale padding; using
        ...
        mma_tiler_sfa = (
            Sm120SwapABSwigluNvfp4Fc12Kernel._round_up_to(mma_tiler_mnk[0], 128),
            mma_tiler_mnk[1],
            mma_tiler_mnk[2],
        )
        mma_tiler_sfb = (
            mma_tiler_mnk[0],
            Sm120SwapABSwigluNvfp4Fc12Kernel._round_up_to(mma_tiler_mnk[1], 128),
            mma_tiler_mnk[2],
        )
    ...
    def get_workspace_size_in_bytes(
        ...
        # FC1 output is packed E2M1: two logical elements per byte.
        fc1_output_bytes = (
            data_total_rows * intermediate_downproj * cutlass.Float4E2M1FN.width
        ) // 8
    ...
    def token_comm_hook_fc1_ready_counter_ptr(self, token_comm_args):
        ...
        ``Sm120SwapABNvfp4Fc12SchedExtension``) should watch as the
    ...
    def _launch_sm120_inline_fc12(
        ...
        loads activation/SFB, and compute warps 0-3 execute ldmatrix + NVFP4
        ...
        if cutlass.const_expr(self.sf_dtype is not cutlass.Float8E4M3FN):
            raise TypeError(
                "SM120 NVFP4 scale tensors must use Float8E4M3FN, got "
                f"{self.sf_dtype}."
            )
        if cutlass.const_expr(
            self.fc1_output_dtype is not cutlass.Float4E2M1FN
        ):
            raise TypeError(
                "SM120 NVFP4 FC1 output must use Float4E2M1FN, got "
                f"{self.fc1_output_dtype}."
            )
        ...
        fc1_output_smem_layout_staged = sm100_utils.make_smem_layout_epi(
            ...
            # SM120's 128-bit epilogue swizzle requires at least N32.  The
            # compact split-K2 N16 specialization does not execute FC1, but
            # the shared call signature still materializes this layout.
            (max(self.mma_tiler_mnk[1], 32), self.mma_tiler_mnk[0] // 2),
            1,
        )
        ...
        sched_ext = Sm120SwapABNvfp4Fc12SchedExtension(
            sf_vec_size=self.sf_vec_size,
    ...
    def sm120_inline_fc12_kernel(
        ...
        sFc1OutputBytes = cute.make_tensor(
            cute.recast_ptr(sFc1Output.iterator, dtype=cutlass.Uint8),
            cute.make_layout(
                (self.mma_tiler_mnk[1], self.mma_tiler_mnk[0] // 4),
                stride=(self.mma_tiler_mnk[0] // 4, 1),
            ),
        )
        ...
        tCrSFA = partition_fragment_sfa_for_sm120_mma(
            tiled_mma, sSFA[None, None, 0], mma_tidx
        )
        tCrSFB = partition_fragment_sfb_for_sm120_mma(
            tiled_mma, sSFB[None, None, 0], mma_tidx
        )
        ...
        if warp_idx < cutlass.Int32(4):
            ...
            while work_tile_info.is_valid_tile:
                ...
                for k_tile in cutlass.range(0, k_tile_cnt, 1, unroll=1):
                    ...
                    for k_inner_mma in cutlass.range_constexpr(0, 2):
                        if cutlass.const_expr(k_inner_mma + 1 < 2):
                            ...
                            cute.copy(
                                smem_tiled_copy_SFA,
                                tCsSFA_p_filtered[None, None, k_inner_next],
                ...
                if is_phase_linear1:
                    ...
                    warp_pair = compute_warp // cutlass.Int32(2)
                    sf_col = (
                        fc1_col_base // cutlass.Int32(self.sf_vec_size)
                        + warp_pair
                    )
                    fc1_alpha_value = cutlass.Float32(1.0)
                    fc1_norm_value = cutlass.Float32(1.0)
                    if cutlass.const_expr(fc1_alpha is not None):
                        fc1_alpha_value = cutlass.Float32(
                            fc1_alpha[work_tile_info.expert_idx]
                        )
                    if cutlass.const_expr(fc1_norm_const is not None):
                        fc1_norm_value = cutlass.Float32(
                            fc1_norm_const[work_tile_info.expert_idx]
                        )
                    ...
                    for ng_pair in cutlass.range_constexpr(0, n_groups // 2):
                        ...
                        gate0 = acc0[0] * fc1_alpha_value
                        gate1 = acc0[1] * fc1_alpha_value
                        gate2 = acc1[0] * fc1_alpha_value
                        gate3 = acc1[1] * fc1_alpha_value
                        up0 = acc0[2] * fc1_alpha_value
                        ...
                        for i in cutlass.range_constexpr(0, 4):
                            ...
                            if lane_g == cutlass.Int32(0):
                                pair_first_warp = warp_pair * cutlass.Int32(2)
                                for warp_offset in cutlass.range_constexpr(0, 2):
                                    warp = pair_first_warp + cutlass.Int32(warp_offset)
                                    amax = cute.arch.fmax(
                                        amax,
                        ...
                        for i in cutlass.range_constexpr(0, 4):
                            ...
                            amax = rAmax[i]
                            raw_scale = cute.arch.fmin(
                                cutlass.Float32(Fp8E4M3FNMax),
                                amax * cutlass.Float32(Nvfp4E2M1RcpLimit)
                                * fc1_norm_value,
                            )
                            scale = cvt_f32_to_f8_to_f32(raw_scale, self.sf_dtype)
                            inv = cutlass.Float32(0.0)
                            if scale > cutlass.Float32(0.0):
                                inv = cute.arch.fmin(
                                    cute.arch.rcp_approx(scale) * fc1_norm_value,
                                    cutlass.Float32(Fp32Max),
                                )
                            if (
                                lane_g == cutlass.Int32(0)
                                and (compute_warp & cutlass.Int32(1))
                                == cutlass.Int32(0)
                            ):
                        ...
                        peer_q0 = cute.arch.shuffle_sync(
                            rFc1OutputLocal[0], partner_lane
                        )
                        peer_q1 = cute.arch.shuffle_sync(
                            rFc1OutputLocal[1], partner_lane
                        )
                        peer_q2 = cute.arch.shuffle_sync(
                            rFc1OutputLocal[2], partner_lane
                        )
                        peer_q3 = cute.arch.shuffle_sync(
                            rFc1OutputLocal[3], partner_lane
                        )
                        if (lane_g & cutlass.Int32(1)) == cutlass.Int32(0):
                            byte_col = (
                                compute_warp * cutlass.Int32(4)
                                + (lane_g >> cutlass.Int32(1))
                            )
                            sFc1OutputBytes[pair_base + token0, byte_col] = pack_e2m1x2(
                                rFc1OutputLocal[0], peer_q0
                            )
                            sFc1OutputBytes[pair_base + token1, byte_col] = pack_e2m1x2(
                                rFc1OutputLocal[1], peer_q1
                            )
                            sFc1OutputBytes[pair_base + token2, byte_col] = pack_e2m1x2(
                                rFc1OutputLocal[2], peer_q2
                            )
                            sFc1OutputBytes[pair_base + token3, byte_col] = pack_e2m1x2(
                                rFc1OutputLocal[3], peer_q3
                            )
                        cute.arch.barrier(
                            barrier_id=self.epilog_sync_bar_id,
                            number_of_threads=32 * len(self.compute_warp_id),
                    ...
                    chunks_per_row = downproj_per_cta // 8
                    store_iters = (
                    ...
                    for store_iter in cutlass.range_constexpr(0, store_iters):
                        ...
                        scratch_ptr = (
                            sFc1OutputBytes.iterator
                            + store_token * cutlass.Int32(downproj_per_cta // 2)
                            + store_byte
                        ).align(4)
                        scratch_i32 = cute.make_tensor(
                            cute.recast_ptr(scratch_ptr, dtype=cutlass.Int32),
                            cute.make_layout(1),
                        )
                        if store_token < work_tile_info.valid_tokens_in_tile:
                            ...
                            output_col = fc1_col_base + store_byte * cutlass.Int32(2)
                            output_ptr = (
                                cute.recast_ptr(
                                    fc1_output.iterator,
                                    dtype=cutlass.Uint8,
                                ...
                                + output_row
                                * (fc1_output.stride[0] // cutlass.Int32(2))
                                + output_col // cutlass.Int32(2)
                            ).align(4)
                    ...
                    if cutlass.const_expr(fc2_alpha is not None):
                        fc2_alpha_value = cutlass.Float32(
                            fc2_alpha[work_tile_info.expert_idx]
                        )
                    for ng in cutlass.range_constexpr(0, n_groups):
                        acc = accs[ng]
                        ...
                        if token0 < work_tile_info.valid_tokens_in_tile:
                            fc2_output[row0, 0, hidden0] = (acc[0] * fc2_alpha_value).to(
                                self.fc2_output_dtype
                            )
                            fc2_output[row0, 0, hidden1] = (acc[2] * fc2_alpha_value).to(
                                self.fc2_output_dtype
                            )
                        if token1 < work_tile_info.valid_tokens_in_tile:
                            fc2_output[row1, 0, hidden0] = (acc[1] * fc2_alpha_value).to(
                                self.fc2_output_dtype
                            )
                            fc2_output[row1, 0, hidden1] = (acc[3] * fc2_alpha_value).to(
                                self.fc2_output_dtype
                            )
                work_tile_info = sched_consumer.consume_work()
    ...
    def __call__(
        ...
        activation: cute.Tensor,           # logical (token_sum_padded, hidden) NVFP4
        fc1_weight: cute.Tensor,           # logical (experts, intermediate_gateup, hidden) NVFP4
        ...
        fc1_output: cute.Tensor,         # logical (token_sum_padded, intermediate_downproj) NVFP4
        fc1_output_sf: cute.Tensor,      # (token_sum_padded_sf, intermediate_downproj / sf_vec_size) FP8
        # ── fc2 (Linear2) problem tensors ────────────────────────────────
        fc2_weight: cute.Tensor,          # logical (experts, hidden, intermediate_downproj) NVFP4
        ...
        # all packed NVFP4; activation and fc1_output use the same format;
        # scales are E4M3. ``self.fc1_output_dtype`` selects the FC1 output
        ...
        fc1_output_smem_layout_staged = sm100_utils.make_smem_layout_epi(
            self.fc1_output_dtype,
            utils.LayoutEnum.COL_MAJOR,
            # Keep the unused FC1 epilogue backing layout legal for the
            # split-K2 N16 specialization; K1 continues to use N32 or wider.
            (max(self.mma_tiler[1], 32), self.mma_tiler[0] // 2),
            1,
        )
    ...
    def fc1fc2_kernel_impl(
        ...
        # fc1->fc2 peek active.
        ext = Sm120SwapABNvfp4Fc12SchedExtension(
            sf_vec_size=self.sf_vec_size,
            fc1_done_counter_ptr=fc1_done_counter.iterator,
            fc2_spin_threshold=ext_fc2_spin_threshold,
        ...
        # Store all tokens' four warp-local maxima in the existing 512-float
        # allocation. N <= 128 fits without extra SMEM, and the dense stride
        # avoids mapping different tokens to the same shared-memory bank.
        # The FC2-only specialization does not materialize these views.
        if cutlass.const_expr(self.scheduler_phase_mode != "fc2"):
            ...
            sFc1Swiglu = cute.make_tensor(
                ...
                cute.make_layout((self.mma_tiler[1], 4), stride=(4, 1)),
            )
        ...
        sFc1OutputBytes = cute.make_tensor(
            cute.recast_ptr(sFc1Output.iterator, dtype=cutlass.Uint8),
            cute.make_layout(
                (self.mma_tiler[1], self.mma_tiler[0] // 4),
                stride=(self.mma_tiler[0] // 4, 1),
            ),
        )
        ...
        if warp_idx < len(self.compute_warp_id):
            ...
            tCrB = tiled_mma.make_fragment_B(tCsB[None, None, None, 0])
            tCrSFA = partition_fragment_sfa_for_sm120_mma(
                tiled_mma, sSFA[None, None, 0], tidx
            )
            tCrSFB = partition_fragment_sfb_for_sm120_mma(
                tiled_mma, sSFB[None, None, 0], tidx
            )
            ...
            while work_tile_info.is_valid_tile:
                ...
                if cutlass.const_expr(self.compact_k1):
                    # Two K64 fragments let the next K128 tile's first loads
                    # overlap the current tile's second MMA. Release SMEM
                    # only after every lane has copied both fragments, and
                    # peel the final tile to avoid a reload branch per step.
                    tCrSFB_mma_lo = tCrSFB
                    tCrSFB_mma_hi = tCrSFB
                    sfb_tile_is_hi = cutlass.Boolean(0)
                    if cutlass.const_expr(self.mma_tiler[1] < 128):
                        tCrSFB_mma_hi = cute.make_tensor(
                            tCrSFB.iterator + n_groups // 4, tCrSFB.layout
                        )
                        sfb_tiles_per_tma = 128 // self.mma_tiler[1]
                        sfb_tile_is_hi = work_tile_info.tile_n_idx % cutlass.Int32(
                            sfb_tiles_per_tma
                        ) != cutlass.Int32(0)
                    if k_tile_cnt > 0:
                        stage_ready = peek_a_full_status
                        wait_start = cutlass.Int64(0)
                        time_wait = cutlass.Boolean(0)
                        if cutlass.const_expr(green_trace is not None):
                            if compute_warp == cutlass.Int32(0):
                                if lane_idx == cutlass.Int32(0):
                                    if stage_ready == cutlass.Boolean(0):
                                        ...
                                        if (
                                            ...
                                            wait_start = read_globaltimer()
                                            time_wait = cutlass.Boolean(1)
                                            k2_trace_tma_a_timed_calls += cutlass.Int64(
                                                1
                                            )
                        handle_a = a_consumer.wait_and_advance(stage_ready)
                        if cutlass.const_expr(green_trace is not None):
                            if compute_warp == cutlass.Int32(0):
                                if lane_idx == cutlass.Int32(0):
                                    if time_wait:
                                        k2_trace_tma_a_wait_ns += (
                                            read_globaltimer() - wait_start
                                        )
                        stage_ready = peek_b_full_status
                        wait_start = cutlass.Int64(0)
                        time_wait = cutlass.Boolean(0)
                        if cutlass.const_expr(green_trace is not None):
                            if compute_warp == cutlass.Int32(0):
                                if lane_idx == cutlass.Int32(0):
                                    if stage_ready == cutlass.Boolean(0):
                                        ...
                                        if (
                                            ...
                                            wait_start = read_globaltimer()
                                            time_wait = cutlass.Boolean(1)
                                            k2_trace_tma_b_timed_calls += cutlass.Int64(
                                                1
                                            )
                        handle_b = b_consumer.wait_and_advance(stage_ready)
                        if cutlass.const_expr(green_trace is not None):
                            if compute_warp == cutlass.Int32(0):
                                if lane_idx == cutlass.Int32(0):
                                    if time_wait:
                        ...
                        for k_tile in cutlass.range(0, k_tile_cnt - 1, 1, unroll=1):
                            ...
                            cute.copy(
                                ...
                                tCrSFB_copy_view_filtered[None, 0, None, 1],
                            )
                            for ng in cutlass.range_constexpr(0, n_groups):
                                if sfb_tile_is_hi:
                                    issue_m64n8k64_nvfp4(
                            ...
                            handle_a.release()
                            handle_b.release()
                            stage_ready = a_consumer.try_wait()
                            wait_start = cutlass.Int64(0)
                            time_wait = cutlass.Boolean(0)
                            if cutlass.const_expr(green_trace is not None):
                                if compute_warp == cutlass.Int32(0):
                                    if lane_idx == cutlass.Int32(0):
                                        if stage_ready == cutlass.Boolean(0):
                                            k2_trace_tma_a_wait_calls += cutlass.Int64(
                                                1
                                            )
                                            if (
                                                k_tile
                            ...
                            stage_ready = b_consumer.try_wait()
                            wait_start = cutlass.Int64(0)
                            time_wait = cutlass.Boolean(0)
                            if cutlass.const_expr(green_trace is not None):
                                if compute_warp == cutlass.Int32(0):
                                    if lane_idx == cutlass.Int32(0):
                                        if stage_ready == cutlass.Boolean(0):
                                            k2_trace_tma_b_wait_calls += cutlass.Int64(
                                                1
                                            )
                                            if (
                                                k_tile
                            ...
                            if compute_warp == cutlass.Int32(0):
                                if lane_idx == cutlass.Int32(0):
                                    if k2_trace_a_wait_active:
                                        k2_trace_tma_a_wait_ns += (
                                            read_globaltimer() - k2_trace_a_wait_start
                                        )
                        if trace_k_detail != cutlass.Int32(0):
                            iket.range_pop()
                            if is_phase_linear1:
                            ...
                            if compute_warp == cutlass.Int32(0):
                                if lane_idx == cutlass.Int32(0):
                                    if k2_trace_b_wait_active:
                                        k2_trace_tma_b_wait_ns += (
                                            read_globaltimer() - k2_trace_b_wait_start
                                        )
                        if trace_k_detail != cutlass.Int32(0):
                            iket.range_pop()
                        peek_a_full_status = cutlass.Boolean(1)
                        ...
                        for k_inner_mma in cutlass.range_constexpr(0, 2):
                            if cutlass.const_expr(k_inner_mma + 1 < 2):
                                ...
                                cute.copy(
                                    smem_tiled_copy_SFA,
                                    tCsSFA_p_filtered[None, None, k_inner_next],
                                    tCrSFA_copy_view_filtered[None, None, k_inner_next],
                                )
                                cute.copy(
                                    smem_tiled_copy_SFB,
                                    tCsSFB_p_filtered[None, None, k_inner_next],
                ...
                if self.scheduler_phase_mode != "fc2" and is_phase_linear1:
                    ...
                    warp_pair = compute_warp // cutlass.Int32(2)
                    sf_col = fc1_col_base + warp_pair * cutlass.Int32(16)
                    fc1_alpha_value = cutlass.Float32(1.0)
                    fc1_norm_value = cutlass.Float32(1.0)
                    if cutlass.const_expr(fc1_alpha is not None):
                        fc1_alpha_value = cutlass.Float32(
                    ...
                    # First retain SwiGLU values in the consumed accumulator
                    # slots and publish all warp-local maxima. Keeping each
                    # token's maxima distinct replaces per-N16 rendezvous with
                    # one whole-tile barrier before quantization.
                    for ng_pair in cutlass.range_constexpr(0, n_groups // 2):
                        ...
                        acc1[1] = val3
                        if trace_compute_detail != cutlass.Int32(0):
                            iket.range_push("sm120_fc1_epi_amax_local")
                        amax0 = cute.math.absf(val0)
                        amax1 = cute.math.absf(val1)
                        amax2 = cute.math.absf(val2)
                        amax3 = cute.math.absf(val3)
                        ...
                        if lane_g == cutlass.Int32(0):
                            sFc1Swiglu[tile_token0, compute_warp] = amax0
                            sFc1Swiglu[tile_token1, compute_warp] = amax1
                            sFc1Swiglu[tile_token2, compute_warp] = amax2
                            sFc1Swiglu[tile_token3, compute_warp] = amax3

                        if trace_compute_detail != cutlass.Int32(0):
                            iket.range_pop()

                    if trace_compute_detail != cutlass.Int32(0):
                        iket.range_push("sm120_fc1_epi_amax_sync")
                    cute.arch.barrier(
                        barrier_id=self.epilog_sync_bar_id,
                        number_of_threads=32 * len(self.compute_warp_id),
                    )
                    ...
                    for ng_pair in cutlass.range_constexpr(0, n_groups // 2):
                        ...
                        raw_token3 = tile_token_base + tile_token3
                        if trace_compute_detail != cutlass.Int32(0):
                            iket.range_push("sm120_fc1_epi_amax_pair")
                        ...
                        if lane_g == cutlass.Int32(0):
                            ...
                            for warp_offset in cutlass.range_constexpr(0, 2):
                                warp = pair_first_warp + cutlass.Int32(warp_offset)
                                amax0 = cute.arch.fmax(
                                    amax0, sFc1Swiglu[tile_token0, warp]
                                )
                                amax1 = cute.arch.fmax(
                                    amax1, sFc1Swiglu[tile_token1, warp]
                                )
                                amax2 = cute.arch.fmax(
                                    amax2, sFc1Swiglu[tile_token2, warp]
                                )
                                amax3 = cute.arch.fmax(
                                    amax3, sFc1Swiglu[tile_token3, warp]
                                )
                        ...
                        raw_scale0 = cute.arch.fmin(
                            cutlass.Float32(Fp8E4M3FNMax),
                            amax0 * cutlass.Float32(Nvfp4E2M1RcpLimit)
                            * fc1_norm_value,
                        )
                        raw_scale1 = cute.arch.fmin(
                            cutlass.Float32(Fp8E4M3FNMax),
                            amax1 * cutlass.Float32(Nvfp4E2M1RcpLimit)
                            * fc1_norm_value,
                        )
                        raw_scale2 = cute.arch.fmin(
                            cutlass.Float32(Fp8E4M3FNMax),
                            amax2 * cutlass.Float32(Nvfp4E2M1RcpLimit)
                            * fc1_norm_value,
                        )
                        raw_scale3 = cute.arch.fmin(
                            cutlass.Float32(Fp8E4M3FNMax),
                            amax3 * cutlass.Float32(Nvfp4E2M1RcpLimit)
                            * fc1_norm_value,
                        )
                        scale0 = cvt_f32_to_f8_to_f32(raw_scale0, self.sf_dtype)
                        scale1 = cvt_f32_to_f8_to_f32(raw_scale1, self.sf_dtype)
                        scale2 = cvt_f32_to_f8_to_f32(raw_scale2, self.sf_dtype)
                        scale3 = cvt_f32_to_f8_to_f32(raw_scale3, self.sf_dtype)
                        inv0 = cutlass.Float32(0.0)
                        inv1 = cutlass.Float32(0.0)
                        inv2 = cutlass.Float32(0.0)
                        inv3 = cutlass.Float32(0.0)
                        if scale0 > cutlass.Float32(0.0):
                            inv0 = cute.arch.fmin(
                                cute.arch.rcp_approx(scale0) * fc1_norm_value,
                                cutlass.Float32(Fp32Max),
                            )
                        if scale1 > cutlass.Float32(0.0):
                            inv1 = cute.arch.fmin(
                                cute.arch.rcp_approx(scale1) * fc1_norm_value,
                                cutlass.Float32(Fp32Max),
                            )
                        if scale2 > cutlass.Float32(0.0):
                            inv2 = cute.arch.fmin(
                                cute.arch.rcp_approx(scale2) * fc1_norm_value,
                                cutlass.Float32(Fp32Max),
                            )
                        if scale3 > cutlass.Float32(0.0):
                            inv3 = cute.arch.fmin(
                                cute.arch.rcp_approx(scale3) * fc1_norm_value,
                                cutlass.Float32(Fp32Max),
                            )

                        if (
                            lane_g == cutlass.Int32(0)
                            and (compute_warp & cutlass.Int32(1)) == cutlass.Int32(0)
                        ):
                            if tile_token0 < work_tile_info.valid_tokens_in_tile:
                        ...
                        partner_lane = lane_idx ^ cutlass.Int32(4)
                        peer_q0 = cute.arch.shuffle_sync(q0, partner_lane)
                        peer_q1 = cute.arch.shuffle_sync(q1, partner_lane)
                        peer_q2 = cute.arch.shuffle_sync(q2, partner_lane)
                        peer_q3 = cute.arch.shuffle_sync(q3, partner_lane)
                        if (lane_g & cutlass.Int32(1)) == cutlass.Int32(0):
                            byte_col = (
                                compute_warp * cutlass.Int32(4)
                                + (lane_g >> cutlass.Int32(1))
                            )
                            sFc1OutputBytes[tile_token0, byte_col] = pack_e2m1x2(
                                q0, peer_q0
                            )
                            sFc1OutputBytes[tile_token1, byte_col] = pack_e2m1x2(
                                q1, peer_q1
                            )
                            sFc1OutputBytes[tile_token2, byte_col] = pack_e2m1x2(
                                q2, peer_q2
                            )
                            sFc1OutputBytes[tile_token3, byte_col] = pack_e2m1x2(
                                q3, peer_q3
                            )
                        if trace_compute_detail != cutlass.Int32(0):
                            iket.range_pop()

                    # Packed rows are disjoint across ng_pair iterations.
                    # Join their stores once before the coalesced SMEM drain.
                    if trace_compute_detail != cutlass.Int32(0):
                        iket.range_push("sm120_fc1_epi_pack_smem")
                    cute.arch.barrier(
                        barrier_id=self.epilog_sync_bar_id,
                        number_of_threads=32 * len(self.compute_warp_id),
                    )
                    ...
                    for ng in cutlass.range_constexpr(0, n_groups):
                        ...
                        if cutlass.const_expr(self.fc2_packed_store):
                            partner_lane = lane_idx ^ cutlass.Int32(4)
                            rFc2Store[0] = (acc[0] * fc2_alpha_value).to(self.fc2_output_dtype)
                            rFc2Store[1] = cute.arch.shuffle_sync(
                                acc[0] * fc2_alpha_value, partner_lane
                            ).to(self.fc2_output_dtype)
                            rFc2Store[2] = (acc[2] * fc2_alpha_value).to(self.fc2_output_dtype)
                            rFc2Store[3] = cute.arch.shuffle_sync(
                                acc[2] * fc2_alpha_value, partner_lane
                            ).to(self.fc2_output_dtype)
                            rFc2Store[4] = (acc[1] * fc2_alpha_value).to(self.fc2_output_dtype)
                            rFc2Store[5] = cute.arch.shuffle_sync(
                                acc[1] * fc2_alpha_value, partner_lane
                            ).to(self.fc2_output_dtype)
                            rFc2Store[6] = (acc[3] * fc2_alpha_value).to(self.fc2_output_dtype)
                            rFc2Store[7] = cute.arch.shuffle_sync(
                                acc[3] * fc2_alpha_value, partner_lane
                            ).to(self.fc2_output_dtype)
                        if token0 < work_tile_info.valid_tokens_in_tile:
                            if cutlass.const_expr(self.fc2_packed_store):
                                if (lane_g & cutlass.Int32(1)) == cutlass.Int32(0):
                                ...
                                if cutlass.const_expr(
                                    ...
                                    peer_row0[hidden0] = (acc[0] * fc2_alpha_value).to(self.fc2_output_dtype)
                                    peer_row0[hidden1] = (acc[2] * fc2_alpha_value).to(self.fc2_output_dtype)
                                else:
                                    fc2_output[pool_token0, 0, hidden0] = (acc[0] * fc2_alpha_value).to(
                                        self.fc2_output_dtype
                                    )
                                    fc2_output[pool_token0, 0, hidden1] = (acc[2] * fc2_alpha_value).to(
                                        self.fc2_output_dtype
                                    )
                        if token1 < work_tile_info.valid_tokens_in_tile:
                            if cutlass.const_expr(self.fc2_packed_store):
                                ...
                                if cutlass.const_expr(
                                    ...
                                    peer_row1[hidden0] = (acc[1] * fc2_alpha_value).to(self.fc2_output_dtype)
                                    peer_row1[hidden1] = (acc[3] * fc2_alpha_value).to(self.fc2_output_dtype)
                                else:
                                    fc2_output[pool_token1, 0, hidden0] = (acc[1] * fc2_alpha_value).to(
                                        self.fc2_output_dtype
                                    )
                                    fc2_output[pool_token1, 0, hidden1] = (acc[3] * fc2_alpha_value).to(
                                        self.fc2_output_dtype
                                    )
        ...
        # ════════════════════════════════════════════════════════════════════
        # Dispatch hook (warp 7 for compact K1, otherwise warps 8-11)
        # ════════════════════════════════════════════════════════════════════
        #
        # Compact K2 has no dispatch hook. Compact K1 dispatches from its
        # otherwise idle auxiliary warp 7; ordinary K1 uses warps 8-11.
        # When dispatch is present, the subclass implements the
        ...
        if cutlass.const_expr(self.has_dispatch_warps):
            dispatch_trace_start = cutlass.Int64(0)
            if cutlass.const_expr(green_trace is not None):
                if warp_idx == cutlass.Int32(self.dispatch_warp_id[0]):
                    if cute.arch.lane_idx() == cutlass.Int32(0):
                        dispatch_trace_start = read_globaltimer()
            if warp_idx >= self.dispatch_warp_id[0]:
                if cutlass.const_expr(self.use_warpgroup_reg_realloc):
                    cute.arch.warpgroup_reg_dealloc(self.task_reg_cnt)
                ...
                if cutlass.const_expr(self.token_back_standalone):
                    ...
                    if (
                        ...
                        + self.token_comm.num_dispatch_warps
                    ):
                        self.token_comm_hook_dispatch_warp_body(
                            token_comm_args,
                            token_comm_storage,
                ...
                if warp_idx == cutlass.Int32(self.dispatch_warp_id[0]):
                    if cute.arch.lane_idx() == cutlass.Int32(0):
                        cute.arch.store(
                            ...
                            read_globaltimer() - dispatch_trace_start,
                            scope="gpu",
                        )
