...
class Sm120SysmemTokenInPullTokenBackPush(TokenInPullTokenBackPush):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if num_dispatch_warps not in (1, 2, 4):
            raise ValueError(
                "num_dispatch_warps must be one of 1/2/4, got "
                f"{num_dispatch_warps}."
            )
        # The common implementation derives its work partition and barrier
        # sizes from these per-instance values.
        self.num_dispatch_warps = num_dispatch_warps
        self.num_dispatch_threads = num_dispatch_warps * self.warp_threads
        ...
        while current_expert_idx < Int32(self.num_experts_per_rank):
            ...
            if current_expert_idx < Int32(self.num_experts_per_rank):
                ...
                if cutlass.const_expr(
                    ...
                    # ``tma_store_1d`` is asynchronous.  The original wide
                    # 72-CTA pull happened to leave enough time before FC1
                    # consumed most rows, but a bounded producer prefix can
                    # enqueue many stores per warp.  Drain the bulk group
                    # before publishing this token's FC1-ready arrival.
                    with cute.arch.elect_one():
    ...
    @cute.jit
    def nvlink_barrier_one_thread(
        self,
        nvlink_barrier_signal,
        nvlink_barrier_counter,
        peer_rank_ptr_mapper,
    ):
        """Cross-rank epoch published by one last-completer lane.

        All K2 CTAs have already completed their system-fenced peer stores
        ...
        signal_slot = (
            signal_phase * Int32(self.world_size) + Int32(self.local_rank)
        )
        for peer in cutlass.range_constexpr(0, self.world_size, 1):
            peer_addr = peer_rank_ptr_mapper.map(
                signal_base,
                Int32(peer),
                Int64(signal_slot * Int32(4)),
            )
            stg_b32_raw(peer_addr, target)
        cute.arch.fence_acq_rel_sys()
        cute.arch.atomic_add(
            nvlink_barrier_counter.iterator,
            Int32(1),
